...
const
  ...
  var
    ...
    W, H, Iteration, PolygonsCommitted, FailCount: integer;
    ...
    begin
      // Calculate progress (0.0 to 1.0) based on how much the error has dropped
      Result := 1.0 - (CurrentScore / InitialScore);
      confine(Result);
    end;

    function LogProgress: double;
    begin
  ...
  begin
    ...
    InitialScore := TotalScore(Canvas, Target);
    ...
    WriteLn('Starting reconstruction. Stopping after ', MaxFailures, ' consecutive failures...');

    while FailCount < MaxFailures do
    begin
      Inc(Iteration);
      ...
      begin
        ...
        FailCount := 0; // Reset failures on success

        WriteLn(Format('Iteration %4d | Polygons: %4d | Score: %.2f | Progress: %.2f%%',
          [Iteration, PolygonsCommitted, CurrentScore, Progress * 100]));

        // Save frame based on the number of committed polygons, not iterations
        if (SaveEvery > 0) and (PolygonsCommitted mod SaveEvery = 0) then
        begin
...
begin
  ...
  if ParamCount >= 4 then MaxFailures := StrToIntDef(ParamStr(4), MaxFailures);
